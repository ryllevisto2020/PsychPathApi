...
async function updateTally(id,survey){
    save_rate = (await conn()).query(`UPDATE tbl_pre_survey SET ${survey} = ${survey} + 1 WHERE question_id = ${id}`).then((c)=>{
        console.log("end")
    })
}

async function updateTallyPost(id,survey){
    save_rate = (await conn()).query(`UPDATE tbl_post_survey SET ${survey} = ${survey} + 1 WHERE question_id = ${id}`).then((c)=>{
        console.log("end")
    })
}
...
app.post("/api/registration",async (req,res)=>{
    addAccount = (await conn()).query(
        ...
        `VALUES(null,'${req.body.username}','${req.body.password}','${req.body.name}','${req.body.age}','${req.body.section}','${req.body.gender}')`).then(async (a)=>{
            await conn.end
...
app.post("/api/login",async (req,res)=>{
    loginAccount = (await conn()).query(`SELECT * FROM tbl_account WHERE 
        ...
        acc_password LIKE '${req.body.password}'`).then(async (c)=>{
            if(c.length > 0 ){
                await conn.end
                return res.json({code:200,message:"Accepted!",id:c[0].id});
            }else{
                await conn.end
...
app.post("/api/level/one/save",async (req,res)=>{
    find = (await conn()).query(`SELECT * FROM tbl_lvl_one WHERE acc_id = ${req.body.id}`).then(async (c)=>{
        if(c.length > 0){
            update= (await conn()).query(`
                ...
                save = (await conn()).query(`INSERT INTO tbl_lvl_one (id,acc_id,res_clinical,res_industrial,res_educational	) 
                    VALUES(null,'${req.body.id}','${Math.round((req.body.count.A/18) * 50)}','${Math.round((req.body.count.B/18) * 50)}','${Math.round((req.body.count.C/18) * 50)}')`).then((c)=>{
                        console.log("end")
                    })
                }
            })
...
app.post("/api/level/two/save",async(req,res)=>{
    find = (await conn()).query(`SELECT * FROM tbl_lvl_two WHERE acc_id = ${req.body.id}`).then(async (c)=>{
        if(c.length > 0){
            ...
            res_industrial = '${Math.round((req.body.Industrial/5) * 50)}' WHERE acc_id = ${req.body.id}`).then((c)=>{
                console.log("end")
            })
        }else{
            save = (await conn()).query(`INSERT INTO tbl_lvl_two (id,acc_id,res_clinical,res_educational,res_industrial) 
                VALUES(null,'${req.body.id}','${Math.round((req.body.Clinical/5) * 50)}','${Math.round((req.body.Educational/5) * 50)}','${Math.round((req.body.Industrial/5) * 50)}')`).then((c)=>{
                    console.log("end")
                })
            }
        })
...
app.post("/api/survey/pre",async (req,res)=>{
    ...
    for (let index = 0; index < data.length; index++) {
        if(data[index].rate == '1'){
            ...
            await updateTally(data[index].id,survey).then((c)=>{
                console.log("end")
            })
        }
    ...
    find_interest = (await conn()).query(`SELECT * FROM tbl_career_interest WHERE acc_id = ${req.body.id}`).then(async (c)=>{
        if(c.length > 0){
            update = (await conn()).query(`UPDATE tbl_career_interest SET score = '${Math.round(percentage_set_A/4)}' 
            WHERE acc_id = '${req.body.id}'`).then((c)=>{
                console.log("end")
            })
        }else{
            save_interest = (await conn()).query(`INSERT INTO tbl_career_interest (id,acc_id,score) 
                VALUES(null,'${req.body.id}','${Math.round(percentage_set_A/4)}')`).then((c)=>{
                    console.log("end")
                })
            }
        })

    find_attitude = (await conn()).query(`SELECT * FROM tbl_career_attitude WHERE acc_id = ${req.body.id}`).then(async (c)=>{
        if(c.length > 0){
            ...
            save_attitude = (await conn()).query(`INSERT INTO tbl_career_attitude (id,acc_id,score) 
                VALUES(null,'${req.body.id}','${Math.round(percentage_set_B/5)}')`).then((c)=>{
                    console.log("end")
                })
            }
        })

    find_skills = (await conn()).query(`SELECT * FROM tbl_career_skills WHERE acc_id = ${req.body.id}`).then(async (c)=>{
        if(c.length > 0){
            ...
            save_skills = (await conn()).query(`INSERT INTO tbl_career_skills (id,acc_id,score) 
                VALUES(null,'${req.body.id}','${Math.round(percentage_set_C/6)}')`).then((c)=>{
                    console.log("end")
                })
            }
        })
...
app.post("/api/survey/post",async(req,res)=>{
    ...
    for (let index = 0; index < data.length; index++) {
        if(data[index].rate == '1'){
            const survey = 'survey_rate_1';
            await updateTallyPost(data[index].id,survey).then((c)=>{
                console.log("end")
            })
        }
        if(data[index].rate == '2'){
...
app.post("/api/percentage",async (req,res)=>{
    level_one_percentage = (await conn()).query(`SELECT * FROM tbl_lvl_one WHERE acc_id = ${req.body.id}`).then((c)=>{
        return c
    })
    level_two_percentage = (await conn()).query(`SELECT * FROM tbl_lvl_two WHERE acc_id = ${req.body.id}`).then((c)=>{
        return c
    })

    skills = (await conn()).query(`SELECT * FROM tbl_career_skills WHERE acc_id = ${req.body.id}`).then((c)=>{
        return c
    })
    interest = (await conn()).query(`SELECT * FROM tbl_career_interest WHERE acc_id = ${req.body.id}`).then((c)=>{
        return c
    })
    attitude = (await conn()).query(`SELECT * FROM tbl_career_attitude WHERE acc_id = ${req.body.id}`).then((c)=>{
        return c
    })
...
app.post("/api/level/three/save",async(req,res)=>{
    find = (await conn()).query(`SELECT * FROM tbl_lvl_three WHERE acc_id = ${req.body.id}`).then(async (c)=>{
        if(c.length > 0){
            update = (await conn()).query(`UPDATE tbl_lvl_three SET mode = '${req.body.fields}', score = '${req.body.score}' 
                WHERE acc_id = '${req.body.id}'`).then(()=>{
                    console.log("end")
                })
            }else{
                save = (await conn()).query(`INSERT INTO tbl_lvl_three (id,acc_id,mode,score) 
                    VALUES(null,'${req.body.id}','${req.body.fields}','${req.body.score}')`).then(()=>{
                        console.log("end")
                    })
                }
            })
